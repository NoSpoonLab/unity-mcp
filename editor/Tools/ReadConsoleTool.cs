...
namespace UnityMCP.Bridge.Tools
{
    ...
    public static class ReadConsoleTool
    {
        ...
        /// <summary>
        /// Main entry point for processing ReadConsoleMessage messages.
        /// Works as the interface implementation for tool registration.
        /// </summary>
        /// <param name="message">Message to process</param>
        /// <returns>Processing result</returns>
        public static object HandleMessage(ReadConsoleMessage message)
        {
            try
            {
                if (message == null)
                {
                    UnityMCPEditorLogger.LogError("[ReadConsole] Message is null");
            ...
            {
                var errorMessage = $"[ReadConsole] Error processing message: {ex.Message}";
        ...
        /// <summary>
        /// Original implementation for compatibility with existing code
        /// </summary>
        public static object HandleCommand(ReadConsoleMessage message)
        {
            ...
            string action = message.Action?.ToLower() ?? "get";
            ...
            try
            {
                ...
                else if (action == "get")
                {
                    ...
                    var types = message.Types?.Select(t => t.ToLower()).ToList()
                                ?? new List<string> { "error", "warning", "log" };
                    int? count = message.Count;
                    string filterText = message.FilterText;
                    string sinceTimestampStr = message.SinceTimestamp;
                    string format = (message.Format ?? "detailed").ToLower();
                    bool includeStacktrace = message.IncludeStacktrace;
            ...
            {
                UnityMCPEditorLogger.LogError($"[ReadConsole] Action '{action}' failed: {e}");
        ...
        private static object ClearConsole()
        {
            ...
            {
                UnityMCPEditorLogger.LogError($"[ReadConsole] Failed to clear console: {e}");
        ...
        )
        {
            ...
            {
                UnityMCPEditorLogger.LogError($"[ReadConsole] Error while retrieving log entries: {e}");
            ...
            finally
            {
                ...
                {
                    UnityMCPEditorLogger.LogError($"[ReadConsole] Failed to call EndGettingEntries: {e}");
                    // Don't return error here as we might have valid data, but log it.
                }
            }
            // Take the last count entries if specified
            if (count.HasValue && count.Value > 0)
            {
                formattedEntries = formattedEntries
                    .Skip(Math.Max(0, formattedEntries.Count - count.Value))
                    .ToList();
            }
