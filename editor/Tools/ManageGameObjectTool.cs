...
{
    /// <summary>
    /// Tool to manipulate GameObjects within the current scene (CRUD, search, components).
    /// </summary>
    public static class ManageGameObjectTool
    {
        /// <summary>
        /// Main entry point for processing GameObject management messages.
        /// </summary>
        public static object HandleMessage(ManageGameObjectMessage message)
        {
            try
            {
                if (message == null)
                {
                    UnityMCPEditorLogger.LogError("[ManageGameObject] Message is null");
            ...
            {
                var errorMessage = $"[ManageGameObject] Error processing message: {ex.Message}";
                UnityMCPEditorLogger.LogError(errorMessage);
                return Response.Error(errorMessage);
            }
        }

        public static object HandleCommand(ManageGameObjectMessage message)
        {
            ...
            // Common parameters
            string targetToken = message.Target;
            string searchMethod = message.SearchMethod?.ToLower();

            // --- Redirect to ManageAssetTool para prefabs ---
            string targetPath = targetToken;
            if (!string.IsNullOrEmpty(targetPath) && targetPath.EndsWith(".prefab", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    UnityMCPEditorLogger.Log($"[ManageGameObject->ManageAsset] Redirecting action '{action}' for prefab '{targetPath}' to ManageAssetTool.");
                    var assetMessage = new ManageAssetMessage
                    {
                        Action = "modify",
                        Path = targetPath,
                        Properties = action == "set_component_property" && !string.IsNullOrEmpty(message.ComponentName) && message.ComponentProperties != null && message.ComponentProperties.ContainsKey(message.ComponentName)
                            ? new Dictionary<string, object> { { message.ComponentName, message.ComponentProperties[message.ComponentName] } }
                            : message.ComponentProperties?.ToDictionary(kvp => kvp.Key, kvp => (object)kvp.Value)
                    };
                    return ManageAssetTool.HandleCommand(assetMessage);
                }
                else if (action == "delete" || action == "add_component" || action == "remove_component" || action == "get_components")
                {
                    return Response.Error($"The action '{action}' on a prefab ('{targetPath}') must be performed using the 'manage_asset' command.");
                }
                // Permitir 'create' y 'find' en prefabs
            }
            // --- Fin redirect ---
            ...
            try
            {
                ...
                {
                    case "create":
                        return CreateGameObject(message);
                    case "modify":
                        return ModifyGameObject(message, targetToken, searchMethod);
                    case "delete":
                        return DeleteGameObject(targetToken, searchMethod);
                    case "find":
                        return FindGameObjects(message, targetToken, searchMethod);
                    case "get_components":
                        string getCompTarget = targetToken;
                        if (getCompTarget == null)
                            return Response.Error("The 'target' parameter is required for get_components.");
                        return GetComponentsFromTarget(getCompTarget, searchMethod);
                    case "add_component":
                        return AddComponentToTarget(message, targetToken, searchMethod);
                    case "remove_component":
                        return RemoveComponentFromTarget(message, targetToken, searchMethod);
                    case "set_component_property":
                        return SetComponentPropertyOnTarget(message, targetToken, searchMethod);
                    default:
            ...
            {
                UnityMCPEditorLogger.LogError($"[ManageGameObject] Action '{action}' failed: {e}");
                return Response.Error($"Internal error processing action '{action}': {e.Message}");
            }
        }

        private static object CreateGameObject(ManageGameObjectMessage @params)
        {
            ...
            bool saveAsPrefab = @params.SaveAsPrefab;
            string prefabPath = @params.PrefabPath;
            string tag = @params.Tag; // Get tag for creation
            string primitiveType = @params.PrimitiveType; // Keep primitiveType check
            ...
            // Set Parent
            string parentToken = @params.Parent;
            ...
            // Set Transform
            Vector3? position = @params.Position != null && @params.Position.Count == 3 
                ? new Vector3(@params.Position[0], @params.Position[1], @params.Position[2]) 
                : null;
            Vector3? rotation = @params.Rotation != null && @params.Rotation.Count == 3 
                ? new Vector3(@params.Rotation[0], @params.Rotation[1], @params.Rotation[2]) 
                : null;
            Vector3? scale = @params.Scale != null && @params.Scale.Count == 3 
                ? new Vector3(@params.Scale[0], @params.Scale[1], @params.Scale[2]) 
                : null;
            ...
            // Add Components
            if (@params.ComponentsToAdd != null && @params.ComponentsToAdd.Count > 0)
            {
                foreach (var typeName in @params.ComponentsToAdd)
                {
                    if (!string.IsNullOrEmpty(typeName))
                    {
                        // Para esta versión simplificada, asumimos que no hay propiedades para los componentes añadidos
                        // durante la creación. Las propiedades se configurarían con una llamada posterior.
                        var addResult = AddComponentInternal(newGo, typeName, null);
                    ...
                    {
                        Debug.LogWarning(
                            $"[ManageGameObject] Invalid component format in componentsToAdd: {typeName}"
                        );
                    }
                }
            }
        ...
        private static object ModifyGameObject(
            ManageGameObjectMessage @params,
            string targetToken,
            string searchMethod
        )
        {
            GameObject targetGo = FindObjectInternal(targetToken, searchMethod, null);
            if (targetGo == null)
            {
                return Response.Error(
            ...
            if (parentToken != null)
            {
                GameObject newParentGo = FindObjectInternal(parentToken, "by_id_or_name_or_path", null);
                if (newParentGo == null && !string.IsNullOrEmpty(parentToken))
                {
            ...
            // Set Active State
            bool? setActive = @params.SetActive;
            ...
            // Remove Components
            if (@params.ComponentsToRemove != null && @params.ComponentsToRemove.Count > 0)
            {
                foreach (var typeName in @params.ComponentsToRemove)
                {
                    if (!string.IsNullOrEmpty(typeName))
                    {
            ...
            if (@params.ComponentsToAdd != null && @params.ComponentsToAdd.Count > 0)
            {
                foreach (var typeName in @params.ComponentsToAdd)
                {
                    if (!string.IsNullOrEmpty(typeName))
                    {
                        var addResult = AddComponentInternal(targetGo, typeName, null);
                        if (addResult != null)
                            return addResult;
                        modified = true;
                    }
            ...
            if (@params.ComponentProperties != null)
            {
                foreach (var entry in @params.ComponentProperties)
                {
                    string compName = entry.Key;
                    var propertiesToSet = entry.Value;
                    if (propertiesToSet != null)
                    {
                        var setResult = SetComponentPropertiesInternal(
                            targetGo,
                            compName,
                            JObject.FromObject(propertiesToSet)
                        );
                        if (setResult != null)
                            return setResult;
                        modified = true;
        ...
        private static object DeleteGameObject(string targetToken, string searchMethod)
        {
            // Find potentially multiple objects if name/tag search is used without find_all=false implicitly
            List<GameObject> targets = FindObjectsInternal(targetToken, searchMethod, true, null); // find_all=true for delete safety
        ...
        {
            GameObject targetGo = FindObjectInternal(target, searchMethod, null);
            if (targetGo == null)
            {
                return Response.Error(
        ...
        )
        {
            ...
            {
                typeName = @params.ComponentName;
                if (@params.ComponentProperties != null && @params.ComponentProperties.ContainsKey(typeName))
                {
                    properties = JObject.FromObject(@params.ComponentProperties[typeName]);
                }
            }
            else if (@params.ComponentsToAdd != null && @params.ComponentsToAdd.Count > 0)
            {
                typeName = @params.ComponentsToAdd[0];
            }
        ...
        )
        {
            ...
            else if (@params.ComponentsToRemove != null && @params.ComponentsToRemove.Count > 0)
            {
                typeName = @params.ComponentsToRemove[0];
            }
        ...
        )
        {
            ...
            string compName = @params.ComponentName;
            ...
            if (!string.IsNullOrEmpty(compName))
            {
                ...
                if (@params.ComponentProperties != null)
                {
                    if (@params.ComponentProperties.TryGetValue(compName, out propertiesToSet))
                    {
                        // Ya tenemos las propiedades para este componente
                    }
                    else if (@params.ComponentProperties.Count > 0)
                    {
                        // Si no hay propiedades específicas para este componente pero hay otras propiedades,
                        // asumimos que todas las propiedades son para este componente
                        propertiesToSet = @params.ComponentProperties.Values.FirstOrDefault();
                    }
                }
            }
            else
            ...
            var setResult = SetComponentPropertiesInternal(targetGo, compName, JObject.FromObject(propertiesToSet));
            if (setResult != null)
        ...
        )
        {
            ...
            string searchTerm = findParams?.SearchTerm ?? targetToken; // Use searchTerm if provided, else the target itself
            bool searchInChildren = findParams?.SearchInChildren ?? false;
            bool searchInactive = findParams?.SearchInactive ?? false;

            // Default search method if not specified
            if (string.IsNullOrEmpty(searchMethod))
            {
                if (int.TryParse(targetToken, out _))
                    searchMethod = "by_id";
            ...
            try
            {
                ...
                if (typeof(UnityEngine.Object).IsAssignableFrom(targetType))
                {
                    ...
                    if (token is JObject refObject)
                    {
                        ...
                        var findParams = new ManageGameObjectMessage
                        {
                            SearchInactive = true,
                            SearchMethod = findMethod,
                            Target = findToken.ToString()
                        };
                        GameObject foundGo = FindObjectInternal(findToken.ToString(), findMethod, findParams);
